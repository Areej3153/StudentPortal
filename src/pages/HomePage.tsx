...
const HomePage = () => {
    const [cookies, setCookie, removeCookie] = useCookies(['jwt','name','photo', 'levelStudy', 'program', 'faculity']);
    const navigate = useNavigate();
    const handleCookie = () => {
        removeCookie('jwt')
        removeCookie('name')
        removeCookie('photo')
        removeCookie('levelStudy')
        removeCookie('program')
        removeCookie('faculity')
        navigate('/')
    }
  ...
  <div className="flex-none">
    ...
    <div className="dropdown dropdown-end">
      <label tabIndex={0} className="btn btn-ghost btn-circle avatar">
        ...
        <img src={cookies.photo} alt='avatar'  />
        </div>
      </label>
      <ul tabIndex={0} className="menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow bg-base-100 rounded-box w-52">
        ...
        <li><a className="justify-between">Settings
        <span className="badge">coming...</span>
            </a>
            </li>
        <li><a  onClick={handleCookie}>Logout</a></li>
      </ul>
    </div>
  </div>

</div>
<div className='container bg-base-100 rounded-lg shadow-md mb-6 flex-1 '>
<div className="hero-content flex-col  h-screen">
    <img src={cookies.photo} className="max-w-md  rounded-full shadow-2xl -mt-20" />
    <div>
      <h1 className="text-5xl font-bold mt-4">Welcome back {cookies.name}</h1>
      <p className="py-6 mb-6">We are delighted to welcome you to Areej University, where your energy and excitement enrich our vibrant campus life. Whether you are beginning or continuing your educational journey with us, we look forward to learning, exploring, and growing together.</p>
      <div className="stats shadow">
  
  <div className="stat place-items-center">
    <div className="stat-title">Level of Study</div>
    <div className="stat-value">{cookies.levelStudy}</div>
    
  </div>
  
  <div className="stat place-items-center">
    <div className="stat-title">Program</div>
    <div className="stat-value text-secondary">{cookies.program}</div>
    
  </div>
  
  <div className="stat place-items-center">
    <div className="stat-title">Faculity/Division</div>
    <div className="stat-value">{cookies.faculity}</div>
...
<footer className="footer footer-center p-4 bg-base-100 rounded-lg shadow-md text-base-content">
  <div>
    <p>Copyright © 2023 - All right reserved by Areej</p>
  </div>
</footer>
</div>
  )
